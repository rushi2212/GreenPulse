    ...
    <div className="p-3 sm:p-6 max-w-3xl mx-auto bg-white rounded-lg shadow-lg mt-4 mb-4 sm:mt-6 sm:mb-6">
          ...
          {leaderboard.length > 1 && (
            <div className="flex flex-col items-center">
              ...
              <div className="text-center">
                ...
                <p className="text-green-600 font-bold text-xs sm:text-base">{leaderboard[1].carbonFootprint.toFixed(2)} kg</p>
              </div>
            </div>
          )}
          ...
          {leaderboard.length > 0 && (
            <div className="flex flex-col items-center transform scale-110">
              ...
              <div className="text-center">
                ...
                <p className="text-green-600 font-bold text-sm sm:text-lg">{leaderboard[0].carbonFootprint.toFixed(2)} kg</p>
              </div>
            </div>
          )}
          ...
          {leaderboard.length > 2 && (
            <div className="flex flex-col items-center">
              ...
              <div className="text-center">
                ...
                <p className="text-green-600 font-bold text-xs sm:text-base">{leaderboard[2].carbonFootprint.toFixed(2)} kg</p>
              </div>
            </div>
          )}
        </div>
        ...
        <table className="w-full table-auto border-collapse bg-white">
          <thead>
            <tr className="bg-green-600 text-white">
              <th className="px-2 sm:px-4 py-2 sm:py-3 text-left text-xs sm:text-base">Rank</th>
              <th className="px-2 sm:px-4 py-2 sm:py-3 text-left text-xs sm:text-base">Name</th>
              <th className="px-2 sm:px-4 py-2 sm:py-3 text-right text-xs sm:text-base">CO₂ Saved</th>
              <th className="px-2 sm:px-4 py-2 sm:py-3 text-right hidden sm:table-cell text-xs sm:text-base">vs Avg</th>
            </tr>
          </thead>
          <tbody>
              ...
              return (
                ...
                >
                  ...
                  <td className="border-b border-green-100 px-2 sm:px-4 py-2 sm:py-3 text-xs sm:text-base max-w-24 sm:max-w-full truncate">
                    {isCurrentUser ? (
                      <span className="font-bold text-green-700">{entry.name} <span className="text-xs">(You)</span></span>
                    ) : (
                      entry.name
                    )}
                  </td>
      ...
      <div className="mt-4 sm:mt-6 grid grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-4">
        <div className="bg-green-50 p-2 sm:p-3 rounded-lg text-center">
          <p className="text-xxs sm:text-xs text-green-600">Total Participants</p>
        ...
        <div className="bg-green-50 p-2 sm:p-3 rounded-lg text-center">
          ...
          <p className="text-lg sm:text-xl font-bold">
            {leaderboard.reduce((sum, entry) => sum + entry.carbonFootprint, 0).toFixed(1)} kg
          </p>
        </div>
        <div className="bg-green-50 p-2 sm:p-3 rounded-lg text-center">
          <p className="text-xxs sm:text-xs text-green-600">Average Savings</p>
          <p className="text-lg sm:text-xl font-bold">
            {(leaderboard.reduce((sum, entry) => sum + entry.carbonFootprint, 0) / Math.max(1, leaderboard.length)).toFixed(1)} kg
          </p>
        </div>
        <div className="bg-green-50 p-2 sm:p-3 rounded-lg text-center">
          <p className="text-xxs sm:text-xs text-green-600">Top Saver</p>
          <p className="text-lg sm:text-xl font-bold">
            {leaderboard.length > 0 ? leaderboard[0].carbonFootprint.toFixed(1) : "0.0"} kg
          </p>
        </div>
      </div>
    </div>
